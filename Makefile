...
lint:
	{\
		...
		docker-compose run --rm backend eslint "**/*.js" --ignore-pattern node_modules/;\
		echo '\n\n-------Linting frontend files-------';\
		docker-compose run --rm frontend eslint "**/*.js" --ignore-pattern node_modules/;\
	}
autofix:
	{\
		echo '\n-------Linting backend files-------';\
		docker-compose run --rm backend eslint "**/*.js" --ignore-pattern node_modules/ --fix;\
		echo '\n\n-------Linting frontend files-------';\
		docker-compose run --rm frontend eslint "**/*.js" --ignore-pattern node_modules/ --fix;\
	}
